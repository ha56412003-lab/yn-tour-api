...
// 创建顾客池（每个推荐人独立的顾客列表，避免重用）
function createCustomerPool(size) {
  const pool = [];
  const firstNames = ['张', '李', '王', '刘', '陈', '赵', '孙', '周', '钱', '吴', '郑', '王', '冯', '褚', '卫', '蒋', '沈', '韩', '杨', '朱'];
  const secondChars = ['*', '**'];
  const lastNames = ['先生', '女士', '太太', '经理', '女士', '先生', '女士', '小姐', '先生', '女士'];
  
  for (let i = 0; i < size; i++) {
    const firstName = firstNames[i % firstNames.length];
    const lastName = lastNames[i % lastNames.length];
    const name = firstName + secondChars[i % 2] + lastName;
    pool.push(name);
  }
  return pool;
}
...
async function main() {
  ...
  for (const u of csvData) {
    ...
    const user = await User.create({
      openid: 'case_v2_' + u.name + '_20260417',
      nickname: nickname,
  ...
  // 3. 创建顾客池（每个推荐人独立的顾客列表）
  const customerPool = createCustomerPool(100);
  const customerUsers = [];
  
  console.log('\n--- 创建顾客账户 ---');
  for (let i = 0; i < customerPool.length; i++) {
    const cname = customerPool[i];
    let u = await User.findOne({ nickname: cname, isDistributor: false });
    if (!u) {
      u = await User.create({
        openid: 'case_customer_v2_' + i + '_' + Date.now(),
        nickname: cname,
  ...
  console.log(`顾客池大小: ${customerUsers.length}`);
  ...
  for (const u of csvData) {
    ...
    console.log(`[${referrerName}] ${userNames[referrerName]} 自购订单 ${selfOrder.orderNo}`);
    
    // 4.2 直推订单（从专属顾客池分配，绝不重用）
    if (u.directPush > 0) {
      const myCustomers = [];
      for (let i = 0; i < u.directPush; i++) {
        const customerIdx = customerStartIdx + i;
        const customer = customerUsers[customerIdx % customerUsers.length];
        myCustomers.push({ customer, orderIdx: i + 1 });
      }
      customerStartIdx += u.directPush;
      
      for (const { customer, orderIdx } of myCustomers) {
        // 关键：只设置一次parentId，永不改变
        if (!customer.parentId || customer.parentId.toString() !== referrer._id.toString()) {
        ...
        const order = await Order.create({
          ...
          distribution: {
            isSelfOrder: false,
            referrerId: referrer._id,  // 明确设置为referrer的_id
            isLockedOrder: false,
            directProfit: 0,
            groupBonus: 0,
      ...
      console.log(`[${referrerName}] ${userNames[referrerName]} 直推订单 ${u.directPush} 单 (顾客: ${myCustomers[0].customer.nickname} 等)`);
    }
  }
  ...
  for (const { order, userName, type } of createdOrders) {
    ...
    try {
      ...
      if (result.success) successCount++;
        ...
        failCount++;
        console.log(`  [${order.orderNo}] ${result.message}`);
      }
    } catch (e) {
      failCount++;
  ...
  for (const u of csvData) {
    ...
    console.log(`${u.name}. ${userNames[u.name]} | 总收益¥${user.totalEarnings.toFixed(2)} | 可用¥${user.availableBalance.toFixed(2)} | 冻结¥${user.frozenBalance.toFixed(2)}`);
  }

  // 排行榜
  console.log('\n【佣金排行榜】');
  userResults.sort((a, b) => b.totalEarnings - a.totalEarnings);
  ...
  console.log(`\n【订单】总${allOrders.length}单 已付款${paidOrders.length}单 收入¥${totalRevenue}`);
