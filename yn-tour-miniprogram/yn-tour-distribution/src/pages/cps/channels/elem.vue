<template>
  <view class="container">
    <view class="header">
      ...
      <text class="page-title">饿了么外卖</text>
    ...
    <view class="section">
      ...
      <view class="shop-list" v-else-if="hasData">
        <view class="shop-item" v-for="item in shops" :key="item.id" @click="goToMiniProgram()">
          ...
          <view class="shop-info"><text class="shop-name">{{ item.title }}</text><text class="shop-desc">{{ item.adress }}</text><view class="shop-bottom"><text class="commission">返¥{{ Math.floor(Math.random() * 2 + 1) }}</text></view></view>
        </view>
      </view>
...
const coupons = ref([{id:1,amount:7,minAmount:35,received:false},{id:2,amount:15,minAmount:60,received:false}])
const receiveCoupon = (item) => { if(!item.received){ item.received = true; uni.showToast({title:'红包已领取',icon:'success'}) } }
const shops = ref([])
const goToMiniProgram = () => { uni.navigateToMiniProgram({ appId: 'wxece2a42a6b07ec3', path: 'pages/home/index', fail: () => uni.showToast({title:'请先更新微信',icon:'none'}) }) }
...
.header{background:linear-gradient(135deg,#ff6b9d,#c75cd8);padding:50rpx 30rpx 30rpx;border-radius:0 0 30rpx 30rpx;display:flex;align-items:center}
...
.coupon-item{display:flex;align-items:center;width:260rpx;flex-shrink:0;background:linear-gradient(90deg,#ff6b9d,#c75cd8);border-radius:12rpx;overflow:hidden}
...
.shop-bottom{display:flex;justify-content:space-between;margin-top:10rpx}
...
.city-item.active{background:#ff6b9d;color:#fff}
</style>
